...
mov_imm_to_rm :: 0b1100011 // Immediate to register
...
// ADD SUB CMP imm_to_reg:
asb_imm_to_rm :: 0b100000
asb_imm_to_rm_add :: 0b000 // MOD is 000
asb_imm_to_rm_sub :: 0b101 // MOD is 101
asb_imm_to_rm_cmp :: 0b111 // MOD is 111
...
process_data :: proc(out: ^strings.Builder, data: []byte) -> Error {
	for i := 0; i < len(data); i += 1 {
		...
		case rm_to_reg, add_rm_w_reg:
			...
			continue
		case asb_imm_to_rm:
			// if sw = 11 we have to sign extend a single byte to a word.
			// if sw = 01 we don't have to sign extend -- it /is/ a word of data.
			s := b >> 1 & 0b1
			w := b & 0b1
			i += 1 // consume next byte
			...
			opname: string
			switch b >> 3 & 0b111 {
			case asb_imm_to_rm_add:
				opname = "add"
			case asb_imm_to_rm_sub:
				opname = "sub"
			case asb_imm_to_rm_cmp:
				opname = "cmp"
			}
			dst, src, inc := get_imm_to_rm(s, w, data[i:], false)
			i += inc
		...
		case mov_imm_to_rm:
			...
			// no s in this operation
			dst, src, inc := get_imm_to_rm(0, w, data[i:], true)
			i += inc
...
get_imm_to_rm :: proc(s, w: u8, data: []byte, print_width: bool) -> (dst, src: string, i: int) {
	regTbl := regW if w == 1 else regB
	...
	case 0b00:
		if rm == 0b110 {
			...
			fmt.printf("get_imm_to_rm with rm == 0b110, mistake? byte #%d\n", i)
			return
		}
	...
	// make this nicer, but for now if we have s = 1 then we don't need to read high no matter what w says
	w := s == 1 ? 0 : w
	imm := read_lo_hi(data[i + 1:], w)
	if print_width {
		src = fmt.tprintf("%s %d", w == 1 ? "word" : "byte", imm)
	} else {
		src = fmt.tprintf("%d", imm)
	}
	i += w == 1 ? 2 : 1
